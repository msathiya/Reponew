...
@Component
public class DedicatedClusterBusinessImpl implements DedicatedClusterBusiness {
	...
	public void createServiceInstance(CreateServiceInstanceRequest request) throws ServiceException {
		try {
			String deploymentName = StringUtils.join(ResourceConstant.DEDICATED_CLUSTER_SERVICE_PREFIX,
			...
			operationRequest.setInstanceCount(rEnvironment.getPlanDedicatedClusterInstance());
	...
	public void deleteServiceInstanceBinding(DeleteServiceInstanceBindingRequest request) throws ServiceException {
		try {
			...
			String authToken=CFUtil.login(sEnvironment);
			String appGuid=CFUtil.getAppsGUID(authToken, cfappName, sEnvironment);
			String serviceBindingGUID=request.getBindingId();
			CFUtil.startApplication(authToken, appGuid, sEnvironment,false);
